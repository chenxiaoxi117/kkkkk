...
if __name__ == '__main__':
    ...
    # scheduling_message_to_josn = scheduling_message_to_json(utilization,
    #                                                         update_link_band, scheduling_message)
    # print(scheduling_message_to_josn)

# # 获取前端用户输入的json数组中的数据
# if len(sys.argv) > 1:  # 通过在接口中把用户配置信息以命令行的形式传入，程序中做相应接收即可
#     User_configuration = json.loads(sys.argv[1])
#     # print(User_configuration)
#     scenario = User_configuration["scenario"]
#     User_resource_list = []
#     if scenario == 3:
#         deviceTotal = User_configuration["deviceTotal"]
#         xml_file_path = "../task_generator/src/applications.xml"
#         tree = ET.parse(xml_file_path)
#         root = tree.getroot()
#         for app in User_configuration["appInfo"]:
#             app_type = str(app['category'])
#             for xml_app in root.findall('application'):
#                 if xml_app.find('type').text == app_type:
#                     User_resource_list.append(int(xml_app.find('usage_percentage').text) * deviceTotal *
#                                               int(xml_app.find('request_size').text) / 100)
#         User_resource = sum(User_resource_list)  # 总的ram消耗
#         # print(User_resource)
#
#
# def adaptive_genetic_algorithm(num_tasks, scene, resource_list, resource_sum):
#     population_size = 10
#     max_generation = 10
#     # 若是终端设备赋能和大规模流量调度场景
...
#             max_generation = 5
#         else:
#             request_resource = sum(resource_list)
#             population_size = 4
#             max_generation = 5
#     return population_size, max_generation
