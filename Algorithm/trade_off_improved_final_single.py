...
from ga.trade_off import trade_off
from tools import *


def trade_off_improve(tasks, device_location, link, resource, history_map, link_bandwidth):
    """
    ...
    print(len(sorted_tasks))  # 该长度应与原任务数一致
    ...
    for idx, task in enumerate(sorted_tasks):
        ...
        if selected != -1 and bandwidth_sufficient:
            ...
            for segment in best_sub_path.strip('[]').split('],['):  # 遍历路径上的每段链路
                ...
                # if segment not in bandwidth_map:
                #     print(f"Segment not found in bandwidth_map: {segment}")
...
if __name__ == '__main__':
    ...
    results_file = os.path.join(result_dir, "tradeoff_improved_results.csv")
    retry_results_file = os.path.join(result_dir, "tradeoff_improved_retry_results.csv")
    ...
    tasks_with_results, resource_finish_first, link_band_finish_first = trade_off_improve(tasks_after_long, device_location, link, resource_after_long, history_map,
    ...
    empty_col[:] = None
    trade_off_results = np.hstack([tasks_with_results, empty_col])
    ...
    # trade_off_results = np.array(tasks_with_results, dtype=object)
    # 添加来源标记
