...
def greedy_allocation(tasks, device_location, link, resource, history_map, link_bandwidth):
    """
    贪心算法：对于每个任务，首先找到对应的 Net_device_id，
    然后筛选出满足资源和时延要求的链路，选择总时延最低的节点；
    如果时延相同，则选择负载最低的节点。
    """
    num_tasks = len(tasks)
        ...
        for i, cloud in enumerate(resource_copy):
            ...
            if cloud[7] >= request_size and cloud[8] >= request_size and total_latency <= max_latency:
                ...
                if total_latency < min_latency:
                    ...
                    min_load = history_map[i]
                    selected = i
                    best_path, best_sub_path = path, sub_path
                    best_total_latency = total_latency
                elif total_latency == min_latency:
                    # 在时延相同时选择负载最低的节点
                    if history_map[i] < min_load:
                        min_load = history_map[i]
                        selected = i
                        best_path, best_sub_path = path, sub_path
                        best_total_latency = total_latency
...
def retry_failed_tasks(failed_tasks, device_location, link, resource, link_band, max_retries=3):
    ...
    while retry_count < max_retries and len(failed_tasks) > 0:
        ...
        # 调用遗传算法对失败任务重新调度
        retry_result, _, _ = greedy_allocation(failed_tasks, device_location, link, resource, history_map, link_band)
...
if __name__ == '__main__':
    ...
    results_file = os.path.join(result_dir, "greedy_allocation_results.csv")
    ...
    retry_results_file = os.path.join(result_dir, "greedy_allocation_retry_results.csv")
    ...
    # 执行调度算法
    tasks_with_results, resource_finish_first, link_band_finish_first = greedy_allocation(tasks_after_long, device_location, link,
                                                                  resource_after_long, history_map, link_band)
    # 获取失败任务
    ...
    # 添加来源标记
    tasks_with_results = np.insert(tasks_with_results, 9, "greedy", axis=1)
    combined_results = np.vstack([long_term_tasks_results, tasks_with_results])
    ...
    write_log(log_file, "greedy_allocation", success_rate, elapsed_time, utilization)
