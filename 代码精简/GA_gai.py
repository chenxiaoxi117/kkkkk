...
# 多目标遗传算法（NSGA-II）
def genetic_algorithm(tasks, device_location, link, resource, link_band, generations=1, population_size=4, mutation_rate=0.01):
    ...
    fitness_values = [calculate_fitness(tasks, device_location, link, resource, ind, link_band)[0] for ind in population]
    fronts = non_dominated_sorting(population, fitness_values)
    pareto_solutions = [population[i] for i in fronts[0]]
    pareto_results = [calculate_fitness(tasks, device_location, link, resource, sol, link_band) for sol in pareto_solutions]
...
def retry_failed_tasks(failed_tasks, device_location, link, resource, link_band, max_retries=3):
    ...
    while retry_count < max_retries and len(failed_tasks) > 0:
        pareto_results = genetic_algorithm(failed_tasks, device_location, link, resource, link_band)
...
if __name__ == '__main__':
    ...
    pareto_results = genetic_algorithm(tasks_after_long, device_location, link, resource_after_long, link_band_after_long)
    ...
    for i, (sol, tasks_with_results, _, _) in enumerate(pareto_results):
        fitness = calculate_fitness(tasks_after_long, device_location, link, resource_after_long, sol, link_band_after_long)[0]
